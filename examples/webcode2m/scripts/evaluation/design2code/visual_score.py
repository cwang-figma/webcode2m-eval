...
def calculate_clip_similarity_with_blocks(image_path1, image_path2, blocks1, blocks2, device):
    # Load and preprocess images
    image1 = CLIP_PREPROCESS(rescale_and_mask(image_path1, [block['bbox'] for block in blocks1])).unsqueeze(0).to(device)
    image2 = CLIP_PREPROCESS(rescale_and_mask(image_path2, [block['bbox'] for block in blocks2])).unsqueeze(0).to(device)
...
def visual_score_v3(origin_html, pred_html, origin_screemnshot, pred_screenshot, tmp_dir, device="cpu",debug=False):
    #return sum_sum_areas, final_score, size_score, matched_text_score, position_score, text_color_score, clip_score
    # This will help fix some html syntax error
    global CLIP_MODEL
    ...
    if len(predict_blocks) == 0 or len(original_blocks) == 0:
        print("[Warning] No detected blocks in: ", pred_screenshot, origin_screemnshot) 
    ...
    if len(matched_areas) > 0:
        ...
        final_position_score = np.mean(position_scores)
        final_text_color_score = np.mean(text_color_scores)
        final_clip_score = calculate_clip_similarity_with_blocks(pred_screenshot, origin_screemnshot, predict_blocks, original_blocks, device)
        final_score = 0.2 * (final_size_score + final_matched_text_score + final_position_score + final_text_color_score + final_clip_score)
        return sum_sum_areas, final_score, final_size_score, final_matched_text_score, final_position_score, final_text_color_score, final_clip_score
    else:
        print("[Warning] No matched blocks in: ", pred_screenshot)
        final_clip_score = calculate_clip_similarity_with_blocks(pred_screenshot, origin_screemnshot, predict_blocks, original_blocks, device)
        return 0.0, 0.2 * final_clip_score, 0.0, 0.0, 0.0, 0.0, final_clip_score


if __name__ == "__main__":
    res = visual_score_v3("/data03/starmage/projects/UICoder/outputs/eval_20240506121247/preds/html/12_0.html",
          "/data03/starmage/projects/UICoder/outputs/eval_20240506121247/answers/html/12_0.html",
          "/data03/starmage/projects/UICoder/outputs/eval_20240506121247/preds/screenshot/12_0.png",
          "/data03/starmage/projects/UICoder/outputs/eval_20240506121247/answers/screenshot/12_0.png",
          "/data03/starmage/projects/UICoder/outputs/eval_20240506121247/tmp")
    print(res)
